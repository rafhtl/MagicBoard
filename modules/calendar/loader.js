	...
	for (var i in events){
		...
		if (seconds >= 0){
			if (seconds <= 60*60*5 || seconds >= 60*60*24*2){
				var time_string = moment(startDate).from(moment().format("YYYY-MM-DD")).replace("peste o zi", "m&#226;ine");
			} else {
				var time_string = moment(startDate).from(moment().format("YYYY-MM-DD")).replace("peste o zi", "m&#226;ine");
			}
			if (!e.RRULE){
			...
			if (endSeconds <= 60*60*5 || endSeconds >= 60*60*24*2){
				var time_string = this.shortRunningText + ' ' + moment(endDate).from(moment().format("YYYY-MM-DD")).replace("peste o zi", "ast&#259;zi")
			} else {
				var time_string = this.longRunningText + ' ' + moment(endDate).from(moment().format("YYYY-MM-DD")).replace("peste o zi", "ast&#259;zi");
			}
			if (!e.RRULE){
				this.eventList.push({'description':e.SUMMARY,'seconds':seconds,'days':time_string,'url': url, symbol: this.calendarSymbol});
			}
		...
		if (e.RRULE){
			...
			for (date in dates) {
				...
				if (seconds >= 0) {
					if (seconds <= 60*60*5 || seconds >= 60*60*24*2){
						var time_string = moment(dt).from(moment().format("YYYY-MM-DD")).replace("peste o zi", "m&#226;ine");
					} else {
						var time_string = moment(dt).from(moment().format("YYYY-MM-DD")).replace("peste o zi", "m&#226;ine");
					}
					this.eventList.push({'description':e.SUMMARY,'seconds':seconds,'days':time_string,'url': url, symbol: this.calendarSymbol});
...
calendar.updateCalendar = function (eventList){
	...
	for (var i in eventList){
		...
		row.append($('<td/>').html(e.description.replace('Orthodox Good Friday', 'Vinerea Mare')).addClass('description'));
